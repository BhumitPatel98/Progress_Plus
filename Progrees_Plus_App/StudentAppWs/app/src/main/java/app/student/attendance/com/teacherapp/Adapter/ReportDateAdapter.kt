...
import app.student.attendance.com.teacherapp.Activities.ReportDateActivity
import app.student.attendance.com.teacherapp.Activities.ReportDateDetailActivity
 ...
 */
class ReportDateAdapter(private var dataList: JSONArray, private var mainActivity: ReportDateActivity)
    : RecyclerView.Adapter<ReportDateAdapter.LectureViewHolder>() {
...
//        holder.mBatch_name.text =jsonObject.optString("Batch_Name")
        ...
        holder.mSubmitBtn.setOnClickListener(View.OnClickListener {
            val intent = Intent(holder.mSubmitBtn.context, ReportDateDetailActivity::class.java)
            intent.putExtra("REPORT_DETAIL", jsonObject.toString())
            intent.putExtra("FROM_DATE", mainActivity.fromDate.text.toString())
            intent.putExtra("TO_DATE", mainActivity.toDate.text.toString())
