...
import app.student.attendance.com.teacherapp.Adapter.ReportMonthDetailAdapter
...
class ReportMonthDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        try {
            ...
            month = intent.extras.getString("MONTH")
    ...
    private fun dashboardAPICall() {
        try {
            ProgressDialogUtil.showDialog(this@ReportMonthDetailActivity);
            val stringRequest = object : StringRequest(Request.Method.POST, ApiSettings.REPORT_MONTHWISE_LIST,
                    Response.Listener<String> { response ->
                        try {
                            ...
                            val animation = AnimationUtils.loadLayoutAnimation(this@ReportMonthDetailActivity, resId)
                            lectureList.setLayoutAnimation(animation)
                            lectureList.layoutManager = LinearLayoutManager(this@ReportMonthDetailActivity, LinearLayoutManager.VERTICAL, false);
                            var array = JNObject.optJSONArray("Subject_Report")
                            if (array.length() > 0)
                                lectureList.adapter = ReportMonthDetailAdapter(array, this@ReportMonthDetailActivity)
                            else
                    ...
                    Response.ErrorListener { error ->
                        try {
                            Toast.makeText(this@ReportMonthDetailActivity, error.toString(), Toast.LENGTH_LONG).show()
                            ProgressDialogUtil.hideDialog();
                        } catch (e: Exception) {
                        }
                    }) {
                override fun getParams(): Map<String, String> {
                    ...
                    try {
                        val userDetail = JSONObject(AppPreferences(this@ReportMonthDetailActivity).retrieveUser())
